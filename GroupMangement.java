...
public class GroupMangement implements Runnable {
	volatile Peer[] peersSent;
	volatile ConcurrentHashMap<Peer, TimeStamp> peersRecieved = new ConcurrentHashMap<Peer, TimeStamp>(); //key: the peer that sent the list. value: the time the peer sent the list
	...
	public GroupMangement(Peer[] peersSent, String ipAddress, int port, DatagramSocket udpSock) throws SocketException {
	...
	public void run() {
		try {
			while(running) {
				...
				//DEBUG
//				System.out.println("GroupMangement.java / run() - The message: " + message);
//				System.out.println("GroupMangement.java / run() - Sending...");
...
//				udpSock.send(packet);
				...
				//DEBUG
//				System.out.println("GroupMangement.java / run() - Length of peersSent Peer[] is: " + peersSent.length);
...
//				}
			...
			System.out.println("EXCEPTION ERROR CAUGHT IN GroupMangement.java - run()");
			e.printStackTrace();
		}	
	}
	/*
	...
	public void updatePeerList(Peer peer) {
		if(checkDuplicate(peer)) {
			...
			String[] dateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()).split("[-]|[ ]|[:]");
			peersRecieved.put(peer, new TimeStamp(dateTime[0],dateTime[1],dateTime[2],dateTime[3],dateTime[4],dateTime[5]));			
			//Debug
//			for (Peer eachPeer : peersRecieved.keySet()) {
//				System.out.println("key(): " + eachPeer.address + ":" + eachPeer.port + " and the time: " + peersRecieved.get(eachPeer).print());
//			}	
		}
	}
//	public ConcurrentHashMap<Peer, TimeStamp> getListOfRecievedPeers() {
//		return peersRecieved;
//	}
	/*
	 * checkDuplicate()
	...
	public Peer chooseRandomPeer() {
		...
		// DEBUG
		// System.out.println("GroupMangement.java / chooseRandomPeer() - random number chosen: " + randNum);
		// System.out.println("GroupMangement.java / chooseRandomPeer() - length of peersSent[] " + peersSent.length);
		// System.out.println("GroupMangement.java / chooseRandomPeer() - Peer: " + printPeer(peersSent[randNum]));
