...
public class SnippetMangement implements Runnable{
	Peer[] peersSent;
	...
	static String recievedUDPMessages = "";
	...
	public SnippetMangement(Peer[] peersSent, DatagramSocket udpSock, String snippetIPAddress, int snippetPort) throws SocketException {
	...
	public void run() {
		try {
			while(hasNextLine()) {	// Extremely important line. Thread will not be killed because it's being hung on scanner. 
				...
				//System.out.println("SnippetMangement.java - " + snippetMessageLog);
				System.out.println(snippetMessageLog);
				sendMessage(message);	
			}
			userInput.close();
		} catch (Exception e) {
			System.out.println("SnippetMangement.java - Multithreading Exception caught");
			e.printStackTrace();
		}
	}
	...
	public void retrieveSnippet(String timeStamp, String snippetMessage, String snippetIPAddress, int snippetPort) {
		...
		recievedUDPMessages = recievedUDPMessages + message; //Update our logs to be sent to get report
	}
	public static String getRecievedUDPMessages() {
	...
	/*
	 * Updates peerlist with the latest peer list from GroupMangement thread
	 */
